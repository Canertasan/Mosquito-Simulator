...
class UnlitBlendShader:
    def __init__(self):
        ...
        self.game_objects2: GameObjectSet = set()
        self.game_objects3: GameObjectSet = set()
    ...
    def draw(self, projection, game_objects: Iterable[GameObject] = None, game_objects2: Iterable[GameObject] = None, game_objects3: Iterable[GameObject] = None):
        if game_objects is None:
            game_objects = self.game_objects

        if game_objects2 is None:
            game_objects2 = self.game_objects2

        if game_objects3 is None:
            game_objects3 = self.game_objects2i
        ...
        for game_object in game_objects2:
            # Set the vao
            gl.glBindVertexArray(game_object.obj_data.vao)

            # Set the transform uniform to self.transform
            gl.glUniformMatrix4fv(self.transformation_location, 1, False,
                                  glm.value_ptr(game_object.transform.get_matrix()))
        ...
        for game_object in game_objects3:
            # Set the vao
            gl.glBindVertexArray(game_object.obj_data.vao)
