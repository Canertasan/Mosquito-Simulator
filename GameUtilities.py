...
class Transform:
    ...
    def __init__(self, parent: 'Transform' = None, position=glm.vec3(0.0), scale=glm.vec3(1.0),
    ...
    def get_final_position_matrix(self):
        return self.get_matrix() * glm.vec4(0.0, 0.0, 0.0, 1.0)
...
class AABB:
    ...
    def get_zero_Y(self):
        return (self.min * 1.0) + self.max / 2.0
...
class ObjData:
    def __init__(self, vao: int, positions_buffer: int, texture_positions_buffer: int, normals_buffer: int,
                 meshes: List[Mesh], aabb: AABB):
        self.vao = vao
