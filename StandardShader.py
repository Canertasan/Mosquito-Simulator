...
layout(binding = 2) uniform sampler2D specular_color_sampler;
...
void main()
{
	...
	vec3 ambient = ambient_color * texture(diffuse_color_sampler, texture_position).rgb;

	vec3 norm = normalize(normal);
	vec3 lightDir = normalize(light_position - world_position);
	float diff = max(dot(norm,lightDir), 0.0);
	vec3 diffuse = diffuse_color * diff * texture(diffuse_color_sampler, texture_position).rgb;


	vec3 viewDir = normalize( camera_position - world_position);
	vec3 reflectDir = reflect(-lightDir, norm);
	float spec = pow(max(dot(viewDir,reflectDir), 0.0), shininess);
	vec3 specular = specular_color * spec * texture(specular_color_sampler,texture_position).rgb;
    ...
    def draw(self, projection, view, camera_position, light_position, game_objects: Iterable[GameObject] = None):
